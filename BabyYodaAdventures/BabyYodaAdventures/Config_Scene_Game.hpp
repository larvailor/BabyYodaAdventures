/**
	A configuration file that contains global variables for Scene_Game
*/

#ifndef CONFIG_SCENE_GAME_HPP
#define CONFIG_SCENE_GAME_HPP

constexpr auto SCENE_GAME_KEY_BINDS_PATH = "Config/Scenes/Game/Scene_Game_KeyBinds.ini";
constexpr auto SCENE_GAME_BABY_YODA_TEXTURESHEET_PATH = "Resources/Images/Game/Textures/BabyYoda/Santa.png";
constexpr auto SCENE_GAME_BABY_YODA_ANIMATIONS_PATH = "Config/Scenes/Game/Scene_Game_BabyYoda_Animations.ini";

#endif
